...
```javascript
{
  ...
  deposits:        [{id, name, amount, rate, finalAmount?, openDate, endDate, capitalization, _deleted?}, ...],  // вклады; capitalization: 'monthly'|'end'; finalAmount = вручную заданная сумма на закрытие (rate тогда производная от неё, depositValueAt на endDate возвращает ровно finalAmount)
