...
## Известный открытый дефект (в часть 2 первым делом)

Финальное ревью ветки нашло его последним, и он остался неисправленным осознанно —
процесс не предусматривает второй волны правок после повторного ревью.

`renderDay()` имеет три ранних выхода. Два зовут `renderDeskCtx()`, третий — ветка
пустого дня (`index.html:2428-2431`) — нет:

```js
  if(!allEntries.length){
    list.innerHTML='<div class="empty-day">…</div>';
    return;                    // ← сюда нужен renderDeskCtx() перед return
  }
```

Следствие: на десктопе, находясь на вкладке «День», удаление **последней** траты
текущего дня (свайпом или кнопкой в модалке) оставляет контекстную колонку с
устаревшей суммой и остатком до следующего полного рендера. Тот же класс, что
исправленная находка I3, и её же пробел: новая проверка «колонка следует за
тратой со «Дня»» покрывает только добавление траты в непустой день.

Лечение — одна строка плюс проверка, которая удаляет единственную трату дня и
утверждает, что `#dtc-spent` изменился.
