...
export const FIXTURE = {
  ...
  // «Аренда» — обязательная категория (v1.79.0). Без явной разметки сработала бы
  // миграция старых особых, а особые у e01 есть только в одном месяце — аренда
  // стала бы разовой. Саму миграцию гоняет отдельная сюита.
  catOblig: { cat0001: true },
  expenses: [
  ...
  // Позиционный массив — старый формат: фикстура заодно прогоняет миграцию
  // лимитов на ключи catId (_migrateCatKeyed) на каждом запуске тестов.
